        ...
        const myText = this.add.text(330, 500, 'Insert Command Here', 
            ...
            myText.setInteractive().on('pointerdown', () => {
                this.rexUI.edit(myText, {
                    onClose: () => {
                        if(myText.text === "ls" && !KeyObject.active && !this.registry.get("HasKey1")){

                            KeyObject.setActive(true);
                            KeyObject.alpha = 1;
                            myText.text = "Insert Command Here";
                        } 
                        else if(myText.text === "ls" && KeyObject.active || myText.text === "ls" && this.registry.get("HasKey1")){
                            myText.text = "Insert Command Here";
                        }
                        else if(myText.text === "cd .."){
                            this.scene.start("Level1");
                        } 
                        else if(myText.text !== "cd .." && myText.text !== "cd " + KeyObject.text && myText.text !== "ls" && myText.text !== "mov " + KeyObject.text + " pockets"){
                            myText.text = "Command Not Found";
                        }
                        else if(myText.text === "mov " + KeyObject.text + " pockets"){
                            KeyObject.setActive(false);
                            KeyObject.alpha = 0;
